...
def run_model_HNEMA_DDI(root_prefix, hidden_dim_main, num_heads_main, attnvec_dim_main, rnn_type_main,
                        num_epochs, patience, batch_size, neighbor_samples, repeat, attn_switch_main, rnn_concat_main,#hidden_dim_aux,
                         layer_list, pred_in_dropout, pred_out_dropout, args): #:whether_CCLE, output_concat, args):
...
##    cellline2id_dict = name2id_dict[-3]
  ...
  ##              col_drug_composite_embedding = torch.cat((col_drug_embedding, col_drug_struc_embedding), axis=1)
       ...
       ##     writer.writerows(test_se_label_list)
        ...
        TE_MSE = mean_squared_error(test_se_label_list, test_se_results)
        TE_RMSE = np.sqrt(TE_MSE)
        TE_MAE = mean_absolute_error(test_se_label_list, test_se_results)
        # coefficient and 2-tailed p-value
        TE_PEARSON = scipy.stats.pearsonr(test_se_label_list.reshape(-1), test_se_results.reshape(-1))
...
if __name__ == '__main__':
    ##lr = 0.0005  # 原来是0.005 weight_decay = 0.001 原来0.001 dropout=0.5原0.5
    ##seed=1024
    ...
    run_model_HNEMA_DDI(args.root_prefix, args.hidden_dim_main, args.num_heads_main, args.attnvec_dim_main, args.rnn_type_main, args.epoch,
                        ...
                        args.layer_list, args.pred_in_dropout, args.pred_out_dropout,  args) # args.whether_CCLE, args.output_concat, args)
# args.root_prefix, args.hidden_dim_main,  args.rnn_type_main, args.epoch,
