...
##    adjlists_ua, edge_metapath_indices_list_ua, adjM, type_mask, name2id_dict, train_val_test_drug_drug_samples, train_val_test_drug_drug_labels, all_drug_morgan, cellline_expression = load_HNEMA_DDI_data_te(root_prefix)
    adjlists_ua, edge_metapath_indices_list_ua, adjM, type_mask, name2id_dict, train_val_test_drug_drug_samples, train_val_test_drug_drug_labels, cellline_expression, similar = load_HNEMA_DDI_data_te(root_prefix)
...
##    cellline2id_dict = name2id_dict[-3]
    ...
    for _ in range(repeat):
        ...
        te_net = therapeutic_effect_DNN_predictor(cellline_expression, similar, len(disease2id_dict),
                                                  hidden_dim_main, hidden_dim_aux, te_layer_list,
                                                  output_concat,  pred_out_dropout, pred_in_dropout, whether_CCLE)
        ...
        if only_test == False:
            for epoch in range(num_epochs):
                ...
                for iteration in range(train_sample_idx_generator.num_iterations()):
                    ...
                    train_cellline_idx = [disease2id_dict[i] for i in train_cellline_symbol]
      ...
      ##              col_drug_composite_embedding = torch.cat((col_drug_embedding, col_drug_struc_embedding), axis=1)
                    ...
                    te_output = sigmoid(te_net(row_drug_composite_embedding, col_drug_composite_embedding, train_cellline_idx))##用了sigmoid
          ...
          ##              col_drug_composite_embedding = torch.cat((col_drug_embedding, col_drug_struc_embedding), axis=1)
                        ...
                        te_output = sigmoid(te_net(row_drug_composite_embedding, col_drug_composite_embedding, val_cellline_idx))##用了sigmoid
        ...
        with torch.no_grad():
            for iteration in range(test_sample_idx_generator.num_iterations()):
                ...
                test_cellline_idx = [disease2id_dict[i] for i in test_cellline_symbol]
...
if __name__ == '__main__':
    ...
    run_model_HNEMA_DDI(args.root_prefix, args.hidden_dim_main, args.num_heads_main, args.attnvec_dim_main, args.rnn_type_main,
              ...
              args.hidden_dim_aux,args.layer_list, args.pred_in_dropout, args.pred_out_dropout, args.output_concat, args.whether_CCLE,
              args)
