...
#
def load_HNEMA_DDI_data_te(prefix): # your folder to store the required original data
    ...
    # adjlist03 = adjlist03
    # in_file.close()
    # in_file = open(prefix + '0/0-1-1-1-1-1-0.adjlist', 'r')
    ...
    # in_file.close()
    # in_file = open(prefix + '0/0-1-1-1-1-1-0_idx.pickle', 'rb')
    ...
    adjM = scipy.sparse.load_npz(prefix + 'safe_adjM.npz')#存储了训练所需所有相互作用的邻接矩阵数据 adjM
...
##    in_file.close()
##    in_file = open(prefix + 'atomnum2id_dict.pickle', 'rb')#原子编号到 ID 的映射字典。
##    atomnum2id_dict = pickle.load(in_file)
##    in_file.close()

    train_val_test_drug_drug_samples = np.load(prefix + 'safe_train_val_test_drug_drug_samples.npz')#训练、验证和测试集的药物-药物样本数据。
    train_val_test_drug_drug_labels = np.load(prefix + 'safe_train_val_test_drug_drug_labels.npz')#训练、验证和测试集的药物-药物标签数据。
    # ECFP6
##    all_drug_morgan = scipy.sparse.load_npz(prefix + 'ECFP6_DNN_coomatrix.npz')
    ...
    if os.path.exists(prefix + 'expression_reduced_normalized3.npy'):
        cellline_expression = np.load(prefix + 'expression_reduced_normalized3.npy', allow_pickle=True)
    else:
        cellline_expression = np.zeros((len(disease2id_dict), 512))
    ...
    if os.path.exists(prefix + 'similar.npy'):
        ...
        print('an empty cell data is generated.')

    return [[adjlist00, adjlist01, adjlist04],[adjlist00, adjlist01, adjlist04]], \
           [[idx00, idx01, idx04],[idx00, idx01, idx04]], \
           adjM, type_mask, \
           [drug2id_dict, target2id_dict, disease2id_dict], \
           train_val_test_drug_drug_samples, train_val_test_drug_drug_labels , cellline_expression, similar
    # adjlist00, adjlist01, adjlist02, idx00, idx01, idx02,
...
#数据包括邻接列表、元路径实例、邻接矩阵、节点类型掩码、实体映射字典、药物-药物样本和标签、ECFP6 数据以及细胞系表达数据。
def load_HNEMA_DDI_data_se(prefix): # your folder to store the required original data
    print('the path of source file is :', prefix)
    ...
    # adjlist05 = adjlist05
    # in_file.close()
    in_file = open(prefix + '0/0-se-0safe_side.adjlist', 'r')
    adjlist04 = [line.strip() for line in in_file]
    adjlist04 = adjlist04
    in_file.close()
    ...
    # idx05 = pickle.load(in_file)
    # in_file.close()
    in_file = open(prefix + '0/0-se-0_safe_side_idx.pickle', 'rb')
    idx04 = pickle.load(in_file)
    in_file.close()

    # read adjacency matrix storing the all required interactions required by training读取邻接矩阵，存储训练所需的所有交互
    adjM = scipy.sparse.load_npz(prefix + 'safe_side_adjM.npz')#存储了训练所需所有相互作用的邻接矩阵数据 adjM
    # type_mask is a mask storing all nodes' types
    type_mask = np.load(prefix + 'safe_side_node_types.npy') #节点类型的掩码数据
...
##    in_file.close()
    ...
    train_val_test_drug_drug_samples = np.load(prefix + 'safe_side_train_val_test_drug_drug_samples.npz')#训练、验证和测试集的药物-药物样本数据。
    train_val_test_drug_drug_labels = np.load(prefix + 'safe_side_train_val_test_drug_drug_labels.npz')#训练、验证和测试集的药物-药物标签数据。
    # ECFP6
##    all_drug_morgan = scipy.sparse.load_npz(prefix + 'ECFP6_DNN_coomatrix.npz')
    # CCLE cell line expression data
...
def load_DNN_DDI_data_te(prefix='C:/Users/Administrator/Desktop/Muthene-main/echino_dataset/fold/'): # your folder to store the required original data
    print('the path of source file is :', prefix)
