...
import numpy as np
from model.base_MA import HNEMA_ctr_ntype_specific##, HNEMA_ctr_ntype_specific_transformer
import csv
...
class HNEMA_lp_layer(nn.Module):
    def __init__(self,
        ...
        self.drug_layer = HNEMA_ctr_ntype_specific(num_metapaths_list[0],#获得metapath-specific 节点特征表示 h，以及用于注意力机制的权重 beta
                                                   in_dim,
                                                   num_heads,#
                                                   attn_vec_dim,#
                                                   rnn_type,
        ...
        if (rnn_concat == True):#否应该在 RNN 的输出中使用拼接
            ...
            #self.fc_drug = nn.Linear(in_dim , out_dim, bias=True)#
...
class HNEMA_link_prediction(nn.Module):
    def __init__(self,
        ...
        # HNEMA_lp layers
        self.layer = HNEMA_lp_layer(num_metapaths_list,#用于执行药物相互作用预测的一层操作
                                     hidden_dim,
                                     out_dim,
                                     num_heads,#
                                     attn_vec_dim,#
    ...
    def forward(self, inputs):
        ...
        #     for i, score in enumerate(attention_scores):
        #         # 提取张量中的值并转换为列表
        #         values = score.squeeze().tolist()  # 压缩并转换为普通列表
