...
import numpy as np
from model.Predictor import side_effect_predictor, therapeutic_effect_predictor
from model.link_prediction import link_prediction
from utils.pytorchtools import EarlyStopping
from utils.data import load_data_te
...
def run_model(root_prefix, hidden_dim_main, num_heads_main, attnvec_dim_main, rnn_type_main,
    ...
    adjlists_ua, edge_metapath_indices_list_ua, adjM, type_mask, name2id_dict, train_val_test_drug_drug_samples, train_val_test_drug_drug_labels, disease_gene, similar = load_data_te(root_prefix)
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    # device = torch.device('cpu')
    features_list = []
    in_dims = []
    # based on type mask, to generate one-hot encoding for each type of nodes (drug/target/disease) in the heterogeneous network
    ...
    disease_gene = torch.tensor(disease_gene, dtype=torch.float32).to(device)
    ...
    file_path = './static/casestudy.csv'
    # file.save(file_path)
    ...
    for _ in range(repeat):
        ...
        te_net = therapeutic_effect_predictor(disease_gene, similar, len(disease2id_dict),
        ...
        #print(f"Total number of parameters in model: {tot_params1 + tot_params2}")#print(f"Total number of parameters in main_net: {tot_params1}")print(f"Total number of parameters in te_net: {tot_params2}")

        if only_test == True:
            temp_prefix = './best_model/checkpoint/'
            # change it to your trained model
            model_save_path = temp_prefix + 'checkpoint-safe.pt'
        else:
      ...
      ##              col_drug_composite_embedding = torch.cat((col_drug_embedding, col_drug_struc_embedding), axis=1)
          ...
          ##              col_drug_composite_embedding = torch.cat((col_drug_embedding, col_drug_struc_embedding), axis=1)
                      ...
                      #  te_output = te_net(row_drug_composite_embedding, col_drug_composite_embedding, val_disease_idx)
  ...
  ##              col_drug_composite_embedding = torch.cat((col_drug_embedding, col_drug_struc_embedding), axis=1)
        ...
        ##        print(te_output.shape) #16乘1
            ...
            test_te_result = torch.cat(test_te_result)

            test_te_result = test_te_result.cpu().numpy()
            print(test_te_result)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--root-prefix', type=str,
                    default='./static/syn/', # the folder to store the model input for current independent repeat
    ...
    ap.add_argument('--whether-disease', default=[True, False],
                    help='Whether use the real disease data to replace one-hot numerical encoding')
    ap.add_argument('--hidden-dim-aux', type=int, default=64,
                    help='Dimension of generate ddisese embeddings. Default is 64.')
    ap.add_argument('--layer-list', default=[2048, 1024, 512], ##default = [2048, 1024, 512]
                    help='layer neuron units list for the TE predictor.')
    ap.add_argument('--pred_in_dropout', type=float, default=0.2,
                    help='The input dropout rate of the TE predictor')
    ap.add_argument('--pred_out_dropout', type=float, default=0.5,
                    help='The output dropout rate of the TE predictor')
